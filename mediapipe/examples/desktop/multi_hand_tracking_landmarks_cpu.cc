...
::mediapipe::Status RunMPPGraph() {
  ...
  while (grab_frames) {
    ...
    for (int i = 0 ; i < output_landmarks.size() ; i ++){
    	const ::mediapipe::NormalizedLandmarkList& landmarks = output_landmarks.at(i);
    	std::cout << "Hand " << i << " (" << landmarks.landmark_size() << ")" << std::endl
    			  << landmarks.DebugString() << std::endl;
//			for (int j = 0; j < landmarks.landmark_size(); ++j) {
//				const NormalizedLandmark& landmark = landmarks.landmark(j);
